...
<center>含有 stream 格式的 Content-Type</center>
...
> 注：只有从A到B才会有process-per-site-instance，直接从空白页面输入还是会重新开启一个渲染进程
...
1. https://linkmarket.aliyun.com内新开的页面都是新开一个渲染进程

> 因为连接里面使用了 rel="noopener noreferrer"这个属性。
>
> 这个涉及到安全了，要完整解释起来就话长了，长话短说，先看阿里这个网站的连接是下面这种形式：
>
